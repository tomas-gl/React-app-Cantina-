...
module.exports = class Recipes {
	...
	create(recipeObj) {
		...
		const fields = ['titre', 'description', 'niveau', 'personnes', 'tempsPreparation', 'ingredients', 'etapes'];
		...
		if (!Array.isArray(recipeObj.ingredients)) {
			...
			throw new Error(
				`Tableau "ingredients" invalide. Cela doit être un tableau de sous-tableaux ayant 2 valeurs :\nPar exemple :  [ ["8","feuilles de brick"] , ["1","oeuf"] , ... ]`
			);
		}
	...
	update(recipeObj, fields) {
		...
		// Titre
		if (fields.titre) recipeObj.titre = String(fields.titre);

		// Description
		if (fields.description) recipeObj.description = String(fields.description);

		// Niveaux
		if (fields.niveau) {
			const niveaux = ['padawan', 'jedi', 'maitre'];

			if (!niveaux.includes(fields.niveau)) throw new Error(`Le champs "niveau" doit être l'un des suivants : ${niveaux.join(',')}`);
		...
		// Personnes
		if (fields.personnes) {
			if (!isNumber(fields.personnes)) throw new Error(`Le champs "personnes" doit être un nombre entier`);
		...
		if (fields.tempsPreparation) {
			if (!isNumber(fields.tempsPreparation)) throw new Error(`Le champs "tempsPreparation" doit être un nombre entier`);
	...
	delete(recipeObj) {
		...
		if (recipeIndex < 0) throw new Error(`Rien à supprimer`);
